...
export async function POST(req: NextRequest) {
  try {
    const { codes, userId: bodyUserId } = await req.json();
    ...
    for (const raw of codes as string[]) {
      ...
      if (!code) continue;
      const exists = await prisma.module.findFirst({ where: { code, ownerId: userId } });
      if (!exists) {
        await prisma.module.create({ data: { code, title: code, creditHours: 12, ownerId: userId } });
        created++;
      }
    }
